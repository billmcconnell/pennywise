...
export interface ParsedBofaRow {
...
const REQUIRED_HEADERS = ['Posted Date', 'Payee', 'Amount'];

export function parseBofaCsv(csvText: string): BofaParseResult {
  ...
  const rows: ParsedBofaRow[] = [];
  const errors: BofaParseResult['errors'] = [];

  records.forEach((rec, i) => {
    if (!rec['Posted Date'] && !rec['Payee']) return;

    try {
      const transactionDate = normalizeDate(rec['Posted Date'] ?? '');
      const amount = normalizeAmount(rec['Amount'] ?? '');
      const description = (rec['Payee'] ?? '').trim();
...
// Detects BofA credit card CSV by its Posted Date,Reference Number,Payee header.
export function isBofaContent(buf: Buffer): boolean {
  const head = buf.slice(0, 256).toString('utf8').trimStart();
  return /^Posted Date,Reference Number,Payee/i.test(head);
}
...
// BofA sign: negative = charge (expense), positive = payment/credit.
